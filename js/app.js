class AppModel {
  constructor() {
    this.STORAGE_KEY = "granMenteProgress";
    this.STUDENT_KEY = "granMenteStudent";
    this.DEMO_PROGRESS = {
      surveys: 2,
      xp: 1080,
    ...
    this.architectureLayers = [
      { label: "Frontend", title: "HTML + CSS + JavaScript", detail: "Pantallas responsive, encuesta IA, dashboard, chatbot, panel docente y rutas STEM." },
      { label: "API simulada", title: "/api/recommendations", detail: "Recibe respuestas del estudiante y devuelve perfil STEM." },
      { label: "Motor IA demo", title: "Reglas + puntajes", detail: "Clasifica intereses por palabras clave y calcula madurez vocacional." },
      { label: "Base de datos demo", title: "localStorage", detail: "Guarda alumno, XP, historial, respuestas y perfil dominante en el navegador." },
      { label: "Seguridad demo", title: "Sesion educativa", detail: "Login demo, datos locales, permisos por rol y mensajes claros de privacidad." },
      { label: "Deploy", title: "GitHub Pages", detail: "Estructura estatica lista para publicarse sin servidor ni configuracion." }
    ];
    ...
    this.miniProjects = [
      { title: "Calculadora STEM", area: "programacion", level: "Inicial", time: "30 min", description: "Crear una calculadora visual con HTML, CSS y JavaScript.", deliverable: "Una pagina que suma, resta y muestra resultados." },
      { title: "Mini videojuego de reflejos", area: "programacion", level: "Inicial", time: "45 min", description: "Un boton aparece en lugares distintos y el alumno gana puntos al hacer clic.", deliverable: "Juego con puntos, tiempo y reinicio." },
      { title: "Chatbot de reglas", area: "ia", level: "Intermedio", time: "50 min", description: "Crear un asistente que responde segun palabras clave.", deliverable: "Chat simple que recomienda una ruta STEM." },
      { title: "Simulador de robot", area: "robotica", level: "Intermedio", time: "60 min", description: "Mover un robot en una cuadricula usando botones de direccion.", deliverable: "Robot visual con instrucciones basicas." },
      { title: "Dashboard de habitos", area: "datos", level: "Intermedio", time: "55 min", description: "Registrar horas de estudio y mostrar progreso con barras.", deliverable: "Panel con metricas y recomendaciones." },
      { title: "Experimento de germinacion", area: "ciencia", level: "Inicial", time: "35 min", description: "Comparar el crecimiento de semillas con luz, sombra y poca agua.", deliverable: "Tabla de observacion y conclusion cientifica." },
      { title: "Puente resistente", area: "ingenieria", level: "Inicial", time: "50 min", description: "Disenar un puente con palitos y medir cuanto peso soporta.", deliverable: "Prototipo fisico y registro de prueba." },
      { title: "Mision espacial interactiva", area: "espacio", level: "Inicial", time: "40 min", description: "Elegir combustible, destino y ver si la mision despega.", deliverable: "Simulador de decisiones con resultado final." }
    ];
  }

  loadProgress() {
    try {
      return { ...this.DEMO_PROGRESS, ...JSON.parse(localStorage.getItem(this.STORAGE_KEY)) };
    } catch {
      return { ...this.DEMO_PROGRESS };
    }
  }
  ...
  ensureDemoProgress() {
    ...
    if (!localStorage.getItem(this.STORAGE_KEY) || !Object.keys(stored.scores || {}).length) {
      this.saveProgress(this.DEMO_PROGRESS);
    }
  }
  ...
  getAiReply(text) {
    ...
    const profile = this.profiles[progress.lastProfile] || this.profiles.programacion;
    const maturity = this.getMaturity(progress.surveys);
    const topKey = this.getTopKey(progress.scores);

    if (value.includes("madurez") || value.includes("nivel")) {
      return `${maturity.name}. Has resuelto ${progress.surveys} encuesta(s). Mientras mas encuestas completes, mas precisa sera tu ruta.`;
    }
    if (value.includes("proyecto") || value.includes("hacer")) {
      const project = this.miniProjects.find((item) => item.area === topKey) || this.miniProjects[0];
...
class AppView {
  ...
  init() {
    ...
    this.elements.questionStage = q("#questionStage");
    this.elements.progressBar = q("#progressBar");
    this.elements.progressLabel = q("#progressLabel");
    this.elements.questionCounter = q("#questionCounter");
    this.elements.results = q("#results");
    this.elements.form = q("#loginForm");
    this.elements.userNameLabel = q("#studentNameLabel");
    this.elements.userAvatar = q("#studentAvatar");
    this.elements.userDropdown = q("#userDropdown");
    this.elements.userToggle = q("#studentToggle");
    this.elements.userMenu = q("#studentPanel");
    this.elements.logout = q("#logoutStudent");
    this.elements.stemButtons = Array.from(document.querySelectorAll(".stem-hamburger"));
    this.elements.stemContent = q("#stemContent");
    this.elements.chatForm = q("#chatForm");
    this.elements.chatInput = q("#chatInput");
    this.elements.chatMessages = q("#chatMessages");
    this.elements.projectGrid = q("#projectGrid");
    this.elements.projectIntro = q("#projectIntro");
    this.elements.filters = Array.from(document.querySelectorAll("[data-filter]"));
    this.elements.dashboardRoot = q("#dashboardRoot");
    this.elements.architectureRoot = q("#architectureRoot");
    this.elements.teacherRoot = q("#teacherRoot");
    this.elements.systemRoot = q("#systemRoot");
    this.elements.xpValue = q("#xpValue");
    this.elements.levelValue = q("#levelValue");
    this.elements.menuToggle = q(".menu-toggle");
    this.elements.mainNav = q(".main-nav");
    this.initLogoutModal();
    return this;
  }

  renderSurveyQuestion(state) {
    if (!this.elements.questionStage) return;
    const question = this.model.questions[state.current];
    const progress = Math.round(((state.current + 1) / this.model.questions.length) * 100);
    if (this.elements.progressBar) this.elements.progressBar.style.width = `${progress}%`;
    if (this.elements.progressLabel) this.elements.progressLabel.textContent = `${progress}%`;
    if (this.elements.questionCounter) this.elements.questionCounter.textContent = `Pregunta ${state.current + 1} de ${this.model.questions.length}`;
    this.elements.questionStage.innerHTML = `
      <div class="fade-swap">
        <h3>${question.text}</h3>
  ...
  renderSurveyResults(profile, state, progress) {
    if (!this.elements.results || !this.elements.questionStage) return;
    this.elements.questionStage.innerHTML = `
      <div class="fade-swap">
  ...
  renderStudentPanel(student) {
    if (this.elements.userNameLabel) this.elements.userNameLabel.textContent = student.name;
    if (this.elements.userAvatar) this.elements.userAvatar.textContent = student.name.slice(0, 1).toUpperCase();
    if (this.elements.userToggle) this.elements.userToggle.setAttribute("aria-expanded", "false");
    if (this.elements.userDropdown) this.elements.userDropdown.classList.remove("open");
    document.body.classList.add("logged-in");
    if (this.elements.form) {
      this.elements.form.closest(".login-card").style.display = "none";
  ...
  showLoginForm() {
    ...
    if (this.elements.form) {
      this.elements.form.closest(".login-card").style.display = "grid";
      window.location.hash = "login";
    }
  }

  renderStemRoute(key) {
    if (!this.elements.stemContent) return;
    const route = this.model.stemRoutes[key] || this.model.stemRoutes.ciencia;
    this.elements.stemContent.innerHTML = `
      <div class="fade-swap">
  ...
  addChatMessage(message, isUser = false) {
    ...
    this.elements.chatMessages.insertAdjacentHTML("beforeend", `<div class="${className}">${message}</div>`);
    this.elements.chatMessages.scrollTop = this.elements.chatMessages.scrollHeight;
  }

  renderProjects(filter, progress) {
    if (!this.elements.projectGrid) return;
    const topKey = this.model.getTopKey(progress.scores);
    const recommendedAreas = this.model.getRelatedProjectAreas(topKey);
    const list = this.model.miniProjects.filter((project) => (
      filter === "todos" ||
      project.area === filter ||
      (filter === "recomendados" && recommendedAreas.includes(project.area))
    ));
    if (this.elements.projectIntro) {
      const profile = this.model.profiles[progress.lastProfile] || this.model.profiles[topKey] || this.model.profiles.programacion;
      this.elements.projectIntro.innerHTML = `<strong>Fernando:</strong> segun sus respuestas, su perfil actual es <strong>${profile.name}</strong>. Por eso GRAN MENTE prioriza proyectos de ${recommendedAreas.join(", ")}.`;
    }
    this.elements.projectGrid.innerHTML = list.map((project) => `
  ...
  renderDashboard(progress) {
    if (!this.elements.dashboardRoot) return;
    const maturity = this.model.getMaturity(progress.surveys);
    const topKey = this.model.getTopKey(progress.scores);
    const profile = this.model.profiles[progress.lastProfile] || this.model.profiles[topKey] || this.model.profiles.programacion;
    const totalAnswers = Object.values(progress.scores).reduce((sum, value) => sum + value, 0) || 1;
    const recommendations = this.model.getProgrammingRecommendations(progress.answers);

    this.elements.dashboardRoot.innerHTML = `
  ...
  renderArchitecture() {
    if (!this.elements.architectureRoot) return;
    this.elements.architectureRoot.innerHTML = this.model.architectureLayers.map((layer, index) => `
  ...
  renderTeacherPanel() {
    if (!this.elements.teacherRoot) return;
    const averageMaturity = Math.round((this.model.teacherStudents.reduce((sum, student) => sum + student.maturity, 0) / this.model.teacherStudents.length) * 10) / 10;
    const totalXp = this.model.teacherStudents.reduce((sum, student) => sum + student.xp, 0);
    const topProfiles = [...new Set(this.model.teacherStudents.map((student) => student.profile))].slice(0, 3);

    this.elements.teacherRoot.innerHTML = `
      <section class="dashboard-grid">
        <article class="metric-card"><span>Estudiantes monitoreados</span><strong>${this.model.teacherStudents.length}</strong></article>
      ...
      <section class="glass-card dashboard-block">
        ...
        <div class="table-wrap">
          <table class="teacher-table">
            ...
            <tbody>
              ${this.model.teacherStudents.map((student) => `
                <tr>
                  <td>${student.name}</td>
                  <td>${student.grade}</td>
  ...
  renderSystemSimulation() {
    if (!this.elements.systemRoot) return;
    this.elements.systemRoot.innerHTML = `
      <article class="system-card">
        <p class="eyebrow">APIs simuladas</p>
        <h3>Endpoints del prototipo</h3>
        <div class="endpoint-list">
          ${this.model.simulatedEndpoints.map((endpoint) => `
      ...
      <article class="system-card">
        ...
        <div class="db-list">
          ${this.model.simulatedTables.map((table) => `
            <div>
      ...
      <article class="system-card">
        ...
        <ul class="stem-list">
          ${this.model.securityControls.map((control) => `<li>${control}</li>`).join("")}
        </ul>
      </article>
    `;
  }
  ...
  updateCompactProgress() {
    if (!this.elements.xpValue || !this.elements.levelValue) return;
    const progress = this.model.loadProgress();
    const maturity = this.model.getMaturity(progress.surveys);
    this.elements.xpValue.textContent = progress.xp;
    this.elements.levelValue.textContent = `${maturity.name} · Nivel ${maturity.level}`;
  }

  initLogoutModal() {
    if (this.element("#logoutOverlay")) return;
    ...
    logoutForm?.addEventListener("submit", (event) => {
      ...
      closeModal();
      this.model.clearStudent();
      if (this.logoutOpen) this.logoutOpen();
    });

    this.logoutOpen = openModal;
  }

  openLogoutModal(callback) {
    this.logoutOpen = callback || this.logoutOpen;
    const overlay = this.element("#logoutOverlay");
    if (!overlay) return;
    overlay.hidden = false;
    requestAnimationFrame(() => overlay.classList.add("open"));
  }
...
class AppController {
  constructor() {
    ...
    this.surveyState = { current: 0, scores: {}, answers: [] };
  }

  init() {
    this.view.init();
    this.model.ensureDemoProgress();
    this.view.updateCompactProgress();
    this.setupSurvey();
    this.setupStudentLogin();
    this.setupStemHamburgers();
    this.setupChatbot();
    this.setupProjects();
    this.setupDashboard();
    this.setupArchitecture();
    this.setupTeacherPanel();
    this.setupSystemSimulation();
    this.setupCounters();
    this.setupReveal();
    this.setupMenu();
    this.setupParticles();
  ...
  setupSurvey() {
    if (!this.view.elements.questionStage) return;
    this.surveyState = { current: 0, scores: {}, answers: [] };
    this.view.renderSurveyPlaceholder();
    this.view.renderSurveyQuestion(this.surveyState);

    this.view.elements.questionStage.addEventListener("click", (event) => {
      const button = event.target.closest("[data-key]");
      if (button) {
        const question = this.model.questions[this.surveyState.current];
        const answer = question.options.find((option) => option.key === button.dataset.key);
        this.surveyState.scores[button.dataset.key] = (this.surveyState.scores[button.dataset.key] || 0) + 1;
        this.surveyState.answers.push({ question: question.text, answer: answer?.label || button.dataset.key, key: button.dataset.key });
        this.surveyState.current += 1;
        if (this.surveyState.current < this.model.questions.length) {
          this.view.renderSurveyQuestion(this.surveyState);
        } else {
  ...
  showSurveyResults() {
    const profileKey = this.model.getTopKey(this.surveyState.scores);
    const profile = this.model.profiles[profileKey];
    const progress = this.model.loadProgress();
    progress.surveys += 1;
    progress.xp += 180;
    progress.lastProfile = profileKey;
    progress.answers = [...progress.answers, ...this.surveyState.answers].slice(-30);
    progress.history = [
      ...progress.history,
      { date: new Date().toLocaleDateString("es-PE"), profile: profile.name, main: profileKey }
    ].slice(-8);
    Object.entries(this.surveyState.scores).forEach(([key, value]) => {
      progress.scores[key] = (progress.scores[key] || 0) + value;
    });
    this.model.saveProgress(progress);
    if (this.view.elements.progressBar) this.view.elements.progressBar.style.width = "100%";
    if (this.view.elements.progressLabel) this.view.elements.progressLabel.textContent = "100%";
    if (this.view.elements.questionCounter) this.view.elements.questionCounter.textContent = "Resultado guardado";
    this.view.renderSurveyResults(profile, this.surveyState, progress);
    this.view.updateCompactProgress();
  }

  setupStudentLogin() {
    const form = this.view.elements.form;
    const storedStudent = this.model.loadStudent();

    if (storedStudent) {
      this.view.renderStudentPanel(storedStudent);
    } else if (form) {
    ...
    if (form) {
      form.addEventListener("submit", (event) => {
        ...
        this.model.saveStudent(student);
        this.model.ensureDemoProgress();
        hint.textContent = "Ingreso correcto.";
        this.view.renderStudentPanel(student);
      });
    }

    const toggleDropdown = (event) => {
      event.stopPropagation();
      const userDropdown = this.view.elements.userDropdown;
      if (!userDropdown) return;
      const isOpen = userDropdown.classList.contains("open");
      if (isOpen) {
        userDropdown.classList.remove("open");
        this.view.elements.userToggle.setAttribute("aria-expanded", "false");
      } else {
        userDropdown.classList.add("open");
        this.view.elements.userToggle.setAttribute("aria-expanded", "true");
      }
    };

    this.view.elements.userToggle?.addEventListener("click", toggleDropdown);
    this.view.elements.userAvatar?.addEventListener("click", toggleDropdown);
    this.view.elements.userNameLabel?.addEventListener("click", toggleDropdown);

    this.view.elements.userMenu?.addEventListener("click", (event) => {
      const target = event.target;
      const isDropdownClick = Boolean(target.closest(".user-dropdown"));
      const isTriggerClick = Boolean(target.closest(".user-trigger"));
    ...
    this.view.elements.userDropdown && Array.from(this.view.elements.userDropdown.querySelectorAll(".dropdown-item")).forEach((item) => {
      if (!item.classList.contains("logout")) {
        item.addEventListener("click", () => {
          this.view.elements.userDropdown.classList.remove("open");
          this.view.elements.userToggle.setAttribute("aria-expanded", "false");
        });
      }
    });
    ...
    document.addEventListener("click", (event) => {
      if (!this.view.elements.userToggle || !this.view.elements.userDropdown) return;
      if (!this.view.elements.userToggle.contains(event.target) && !this.view.elements.userDropdown.contains(event.target)) {
        this.view.elements.userDropdown.classList.remove("open");
        this.view.elements.userToggle.setAttribute("aria-expanded", "false");
      }
    });
    ...
    this.view.elements.logout?.addEventListener("click", () => logoutHandler());
  }

  setupStemHamburgers() {
    const buttons = this.view.elements.stemButtons;
    if (!buttons.length || !this.view.elements.stemContent) return;
    ...
    buttons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        this.view.renderStemRoute(button.dataset.stem);
      });
    });

    this.view.renderStemRoute("ciencia");
  }

  setupChatbot() {
    const form = this.view.elements.chatForm;
    if (!form || !this.view.elements.chatInput) return;

    form.addEventListener("submit", (event) => {
      event.preventDefault();
      const text = this.view.elements.chatInput.value.trim();
      if (!text) return;
      this.view.addChatMessage(text, true);
      this.view.elements.chatInput.value = "";
      setTimeout(() => {
        this.view.addChatMessage(this.model.getAiReply(text), false);
      }, 360);
    });
  }
  ...
  setupProjects() {
    const progress = this.model.loadProgress();
    if (!this.view.elements.projectGrid) return;
    this.view.renderProjects("recomendados", progress);
    this.view.elements.filters.forEach((button) => {
      button.addEventListener("click", () => {
        this.view.elements.filters.forEach((item) => item.classList.remove("active"));
        button.classList.add("active");
        this.view.renderProjects(button.dataset.filter, progress);
      });
    });
  }
  ...
  setupDashboard() {
    const progress = this.model.loadProgress();
    this.view.renderDashboard(progress);
  }
  ...
  setupTeacherPanel() {
    this.view.renderTeacherPanel();
  }

  setupSystemSimulation() {
    this.view.renderSystemSimulation();
  }
  ...
  setupMenu() {
    const toggle = this.view.elements.menuToggle;
    const nav = this.view.elements.mainNav;
